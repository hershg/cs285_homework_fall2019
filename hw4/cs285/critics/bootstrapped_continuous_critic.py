    ...
    def update(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if (i % self.num_grad_steps_per_target_update == 0):
                V_s_p = self.forward(next_ob_no) * (1-terminal_n)
                target_n = re_n + self.gamma * V_s_p
            loss, _ = self.sess.run([self.critic_update_op, self.critic_loss], feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: target_n})
        return loss
