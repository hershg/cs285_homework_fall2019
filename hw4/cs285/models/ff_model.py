...
class FFModel(BaseModel):
    ...
    def define_forward_pass(self):
        ...
        obs_normalized = normalize(obs_unnormalized, self.obs_mean_pl, self.obs_std_pl) # (Q1) Define obs_normalized using obs_unnormalized,and self.obs_mean_pl and self.obs_std_pl
        acs_normalized = normalize(acs_unnormalized, self.acs_mean_pl, self.acs_std_pl) # (Q2) Define acs_normalized using acs_unnormalized and self.acs_mean_pl and self.acs_std_pl
        ...
        # (Q1) Use the build_mlp function and the concatenated_input above to define a neural network that predicts unnormalized delta states (i.e. change in state)
        self.delta_pred_normalized = build_mlp(concatenated_input, self.ob_dim, self.scope, self.n_layers, self.size)
        self.delta_pred_unnormalized = unnormalize(self.delta_pred_normalized, self.delta_mean_pl, self.delta_std_pl) # (Q1) Unnormalize the the delta_pred above using the unnormalize function, and self.delta_mean_pl and self.delta_std_pl
        self.next_obs_pred = self.delta_pred_unnormalized + obs_unnormalized # (Q1) Predict next observation using current observation and delta prediction (not that next_obs here is unnormalized)

    def define_train_op(self):

        # normalize the labels
        self.delta_labels_normalized = normalize(self.delta_labels, self.delta_mean_pl, self.delta_std_pl) # (Q1) Define a normalized version of delta_labels using self.delta_labels (which are unnormalized), and self.delta_mean_pl and self.delta_std_pl

        # compared predicted deltas to labels (both should be normalized)
        # (Q1) Define a loss function that takes as input normalized versions of predicted change in state and ground truth change in state
        self.loss = tf.losses.mean_squared_error(self.delta_labels_normalized, self.delta_pred_normalized)
        self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss) # (Q1) Define a train_op to minimize the loss defined above. Adam optimizer will work well.
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        # (Q1) Run model prediction on the given batch of data
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        # train the model
        # (Q1) Run the defined train_op here, and also return the loss being optimized (on this batch of data)
